...
cls()
title_line(' Program księgowo-magazynowy "sprzedaz.py" ')
...
while True:
    if not os.path.exists(logs_file) or len(sys.argv) != 5:
        ...
        command_list.append(['sprzedaż', str(sys.argv[2]),
                             int(sys.argv[3]), int(sys.argv[4])])
        stockStatus.change_buy(str(sys.argv[2]), int(sys.argv[3]), int(sys.argv[4]))
