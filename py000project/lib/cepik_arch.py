...
# CEPIK - baza pojazdów
'''
Lista słowników zawierających dostępne wartości, które można wykorzystać
przy przeszukiwaniu danych API, np przy filtrowaniu, dostępne pod adresem:
"https://api.cepik.gov.pl/www/schema/dictionary-list.json"


"id":         "marka",
"type":       "slowniki",
"links":      {"self":    "https://api.cepik.gov.pl/slowniki/marka"},
"attributes": {"opis-slownika": Wszystkie marki pojazdów występujące w CEPiK"}


"id":         "rodzaj-pojazdu",
"type":       "slowniki",
"links":      {"self":    "https://api.cepik.gov.pl/slowniki/rodzaj-pojazdu"},
"attributes": {"opis-slownika":    "Rodzaje pojazdów występujące w CEPiK"}
'''
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -#
...
class CepikAPIcars:
    '''
        Class representing the CEPIK brands & types vehicles database:
            | self.response_marks - method stores the responses from the server
            | self.response_marks - method stores the responses from the server
            | self.db_marks  - database of brands already downloaded
            | self.db_types  - database of types already downloaded
            | self.counter   - download counter
    '''
    def __init__(self):
        self.response_marks = {}
        self.response_types = {}
    ...
    def get_response(self):
        '''
            Method sends inquiries to the server:
                | url_marks       - html address of the home page
                | url_types       - html address of the home page
            and than saves the answers in a lists & dicts
        '''
